...
import {
  VEGAN_DISTRIBUTOR_DEMO_ROWS,
  VEGAN_DISTRIBUTOR_DEMO_COUNT,
  type VeganDistributorRow,
} from '@/lib/vegan-intelligence-demo-data'

export type { VeganDistributorRow }
...
function DistributorIntelligenceThead() {
  return (
    <thead className="bg-white">
      <tr>
        ...
        <th colSpan={3} className={`${thGroup} bg-[#F0D4BC]`}>
          Product required
        </th>
      ...
      <tr>
        ...
        <th className={thSub}>Distribution Scale</th>
        ...
        <th className={thSub}>End Use Applications</th>
        <th className={thSub}>Omega Concentrations</th>
        <th className={thSub}>Sales Channel Strength</th>
        <th className={thSub}>Key Retail / Network Covered</th>
        <th className={thSub}>Import Capability</th>
      </tr>
    </thead>
  )
}

function DistributorIntelligenceDataRow({ r }: { r: VeganDistributorRow }) {
  return (
    <tr className="odd:bg-white even:bg-[#FAFCFE] hover:bg-[#F0F7FA]">
      ...
      <td className={td}>{r.distributionScale}</td>
      ...
      <td className={td}>{r.endUseApplications}</td>
      <td className={td}>{r.omegaConcentration}</td>
      <td className={td}>{r.salesChannelStrength}</td>
      <td className={td}>{r.keyRetailNetwork}</td>
      <td className={td}>{r.importCapability}</td>
    </tr>
  )
}
...
export function VeganOmegaDistributorIntelligenceTable({ title, height = 600 }: Props) {
  const rows = VEGAN_DISTRIBUTOR_DEMO_ROWS
    ...
    <div className="rounded-lg border border-sky-200/90 shadow-sm overflow-hidden bg-white relative z-0">
      <div className="bg-[#E8F4FA] px-5 py-4 border-b border-sky-200/60">
        <h2 className="text-lg font-bold text-gray-900 tracking-tight">
          {title ||
            `Vegan Omega Oil Ingredient Distributors Intelligence Database (${VEGAN_DISTRIBUTOR_DEMO_COUNT} Distributors across the all Country)`}
        </h2>
        <p className="text-sm text-[#168AAD] mt-1.5 font-medium">
          Verified directory and insight on Vegan Omega Oil Ingredient Distributors across the Country.
        </p>
        <p className="mt-3 text-xs text-gray-600">
          Showing <span className="font-semibold text-gray-800">{rows.length}</span> of {VEGAN_DISTRIBUTOR_DEMO_COUNT}{' '}
      ...
      <div className="rounded-b-md overflow-hidden border-t border-[#C5D3DD]">
        ...
        >
          ...
          <div className="min-w-0">
            <div className="font-bold text-white text-sm">Distributor directory table</div>
        ...
        {tableOpen && (
          <div className="p-3 bg-[#F4F7FA] border-t border-[#C5D3DD]">
            ...
            >
              <table className="min-w-[2400px] w-full text-xs border-collapse">
                <DistributorIntelligenceThead />
                <tbody>
                  {rows.map((r) => (
                    <DistributorIntelligenceDataRow key={r.sNo} r={r} />
                  ))}
                </tbody>
              </table>
            </div>
